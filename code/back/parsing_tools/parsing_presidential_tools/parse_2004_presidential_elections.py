def parse_2004_presidential_elections(region_links_2004):
    """
    Функция для парсинга результатов выборов президента РФ в 2004 г.
    :param region_links_2004: list[str], ссылки на регионы, в которых нужно спарсить результат
    ...
    for i, link in enumerate(region_links_2004):
        print(f'[INFO] Собираю данные про регион №{i} из {len(region_links_2004)} \nURL: {link}')
        ...
        commission_name = soup.find_all('table', attrs={'class': 'table-borderless table-sm',
                                                        'style': "width:100%"})[0].find_all('td')[1].text.strip()
        # дата
        vote_date_raw = soup.find_all('div', attrs={'id': 'election-info'})[0]
        vote_date_full = vote_date_raw.find_all('td')[-1].text.strip()
